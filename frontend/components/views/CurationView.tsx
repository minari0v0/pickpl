...
export default function CurationView({
    ...
    const themeGuideTags = useMemo(() => {
        ...
        switch (theme) {
            ...
            case '웰니스':
                return ['#다도체험', '#조용한사색', '#프라이빗룸', '#힐링무드'];
            case '반려동물':
                return ['#애견동반', '#동반입장', '#넓은운동장', '#안전펜스'];
            case '야시장':
                return ['#야외야장', '#포차감성', '#가성비최고', '#선선한밤바람'];
            default:
    ...
    const themeGuideTips = useMemo(() => {
        ...
        switch (theme) {
            ...
            case '웰니스':
                return [
                    '마음을 차분히 비우고 다도에 집중할 수 있도록 차를 내리는 방법이 안내 책자로 친절히 준비되어 있어요.',
                    '대부분 조용히 사색을 즐기는 공간이므로, 소규모(1~2인)로 조근조근 대화하며 머무르기 좋아요.',
                    '가벼운 가사 없는 클래식이나 가야금 선율이 배경음으로 흘러나와 깊은 힐링을 선사해요.'
                ];
            case '반려동물':
                return [
                    '실내 동반 시 리드줄 착용이나 이동가방(켄넬) 지참이 필수인 곳이 많으니 출발 전 규정을 체크해 보세요.',
                    '반려동물 전용 수제 간식이나 전용 식수대가 구비된 곳이 많아 아이들도 편하게 쉴 수 있어요.',
                    '다른 손님들과의 안전거리를 확보할 수 있는 구석 안쪽 테이블을 선점하시는 것을 추천해 드려요.'
                ];
            case '야시장':
                return [
                    '야외 야장 자리는 인기가 많아 해가 지기 직전인 오후 6시 전후로 방문하시면 대기 없이 앉기 좋아요.',
                    '선선한 밤바람을 막아줄 얇은 겉옷이나 카디건을 챙겨가시면 야외 감성을 더 오래 만끽할 수 있어요.',
                    '주차가 다소 협소한 골목이 많아 가급적 대중교통을 이용해 가볍게 오시는 것이 훨씬 편리해요.'
                ];
            default:
                return [
